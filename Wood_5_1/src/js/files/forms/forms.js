	...
	async function formSubmitAction(form, e) {
		...
		if (error === 0) {
			...
			if (ajax) { // Если режим ajax
				e.preventDefault();
                // к форме добавить data-action="telegram.php"
                // const formAction2 = form.dataset.action ? form.dataset.action.trim() : '#';
                ...
                //Раскомментить
                // const response2 = await fetch(formAction2, {
                //     method: formMethod,
                //     body: formData
                // });
                // ДОбавить || response2.ok
                if (response.ok) {
                    let responseResult = await response.json();
                    // let responseResult2 = await response2.json();
